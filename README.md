# A Node.js Aerocoin Client!

![aerocoin](https://github.com/aeromasterdev/node-aerocoin-master/blob/master/node-aerocoin.png?raw=true)

node-aerocoin is a aerocoin client for Node.js. It is a fork of the excellent Kapitalize Bitcoin Client (now removed from GitHub) intended for use with aerocoin. The purpose of this repository is:

* Provide a one-stop resource for the Node.js developer to get started with aerocoin integration.
* Prevent would-be aerocoin web developers worrying whether a Bitcoin client will work out of the box.
* Promote Node.js development of aerocoin web apps.
* Identify and address any incompatibilities with the aerocoin and Bitcoin APIs that exist now and/or in the future.

## Dependencies

You'll need a running instance of [aerocoind](https://github.com/aeromasterdev/AeroCoin) to connect with.

Then, install the node-aerocoin NPM package.

`npm install https://github.com/aeromasterdev/node-aerocoin`

## Examples

Some code examples follow below, but for more complete examples, see [these snippets](https://github.com/aeromasterdev/node-aerocoin/aerocoin-code-snippets)

```javascript

    var aerocoin = require('node-aerocoin')()

    aerocoin.auth('myusername', 'mypassword')

    aerocoin.getDifficulty(function() {
...
```javascript
        ...
        aerocoin.getNewAddress(function(err, address) {
...
```javascript
    ...
    aerocoin.exec('getNewAddress')

    aerocoin.exec('getbalance', function(err, balance) {
...
<td> <b>Currently only available on testnet</b> Add a nrequired-to-sign multisignature address to the wallet. Each key is a aerocoin address or hex-encoded public key. If [account] is specified, assign address to [account]. </td>
<td> N
</td></tr>
<tr>
<td> backupwallet </td>
...
<tr>
<td> dumpprivkey </td>
<td> [aerocoinaddress] </td>
<td> Reveals the private key corresponding to <aerocoinaddress< </td>
<td> Y
</td></tr>
<tr>
<td> encryptwallet </td>
...
<td> [account] </td>
<td> Returns the current aerocoin address for receiving payments to this account. </td>
<td> N
</td></tr>
<tr>
...
<td> getgenerate </td>
<td> </td>
<td> Returns true or false whether aerocoind is currently generating hashes </td>
<td> N
</td></tr>
<tr>
...
<td> getnewaddress </td>
<td> [account] </td>
<td> Returns a new aerocoin address for receiving payments.  If [account] is specified (recommended), it is added to the address book so payments received with the address will be credited to [account]. </td>
<td> N
</td></tr>
<tr>
...
<td> [aerocoinaddress] [minconf=1] </td>
<td> Returns the total amount received by <aerocoinaddress< in transactions with at least [minconf] confirmations. While some might consider this obvious, value reported by this only considers *receiving* transactions. It does not check payments that have been made *from* this address. In other words, this is not "getaddressbalance". Works only for addresses in the local wallet, external addresses will always show 0. </td>
<td> N
</td></tr>
<tr>
...
<p>To get a list of accounts on the system, execute aerocoind listreceivedbyaddress 0 true
</p>
</td>
<td> N
</td></tr>
...
<tr>
<td> sendfrom </td>
<td> [fromaccount] [toaerocoinaddress] [amount] [minconf=1] [comment] [comment-to] </td>
...
<td> stop </td>
<td> </td>
<td> Stop aerocoin server. </td>
<td> N
</td></tr>
<tr>
<td> validateaddress </td>
<td> [aerocoinaddress] </td>
<td> Return information about [aerocoinaddress]. </td>
<td> N
</td></tr>
<tr>
<td> verifymessage </td>
<td> [aerocoinaddress] [signature] [message] </td>
...
var aerocoin = require('aerocoin')({
    user:'user'
})

aerocoin.set('pass', 'somn')
aerocoin.set({port:7142})
...
You may pass an optional function `passphrasecallback` to the `node-aerocoin` initialization function to manage wallet unlocks. `passphrasecallback` should be a function accepting three arguments:
...
By default `aerocoind` exposes its JSON-RPC interface via HTTP; that is, all RPC commands are transmitted in plain text across the network! To secure the JSON-RPC channel you can supply `aerocoind` with a self-signed SSL certificate and an associated private key to enable HTTPS. For example, in your `aerocoin.conf`:

    rpcssl=1
    rpcsslcertificatechainfile=/etc/ssl/certs/aerocoind.crt
    rpcsslprivatekeyfile=/etc/ssl/private/aerocoind.pem

In order to securely access an SSL encrypted JSON-RPC interface you need a copy of the self-signed certificate from the server: in this case `aerocoind.crt`. Pass your self-signed certificate in the `ca` option and set `https: true` and node-aerocoin is secured!
    
```js
var fs = require('fs')

var ca = fs.readFileSync('aerocoind.crt')
...
nodeunit test/test-node-aerocoin.js
```

## Bounties

[aerocoin](http://www.aerocoin.com) donation address is AdZp6WnHJ5L9iRNBByWPceqANKeityo2rv

Donations in [aerocoin](http://aerocoin.org) will be used for bounties. The first bounty will be awarded for creating a unit test suite. As a side note: I encourage all GitHub repository owners to post a donation address so their community can easily support development financially. 
